...
├── pipeline/
│   └── production-pipeline.md   # 기획→제작→업로드→분석 파이프라인
├── prompts/
│   ├── chatgpt/                 # ChatGPT 업무별 프롬프트 템플릿
│   └── higgsfield/              # Higgsfield 영상 생성 프롬프트 템플릿 (장르별)
├── database/                    # 콘텐츠 기획/제작/성과 추적 DB (CSV)
│   ├── content_ideas.csv
...
5. 매주 성과 데이터를 검토하여 `strategy/02-recommended-strategy.md` 를 갱신한다 (성과 좋은 요소 확대 / 낮은 요소 폐기).
